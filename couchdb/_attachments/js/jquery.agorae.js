(function ($) {
  ...
  $.extend($.agorae, {
    ...
    httpSend: function (url, options) {
      ...
      if (
        typeof options.processData == "boolean" &&
      ...
      processData =
        typeof options.processData != "boolean" ? false : options.processData;
      dataType =
        typeof options.dataType == "string" ? options.dataType : "json";
      headers =
        options.anonymous || $.agorae.session.username === undefined
          ? {}
          : {
              Authorization:
                "Basic " +
                btoa(
                  $.agorae.session.username + ":" + $.agorae.session.password
                ),
            };
      ...
      $.ajax({
        ...
        success: function (resp) {
          ...
          if (options.cache !== false) $.agorae.cache[url] = resp;
        ...
        error: function (req) {
          ...
          if (options.error) {
            ...
            if (req.status == 0) reason = "cross domain request limited";
    ...
    loadConfig: function (configDocUrl) {
      $.agorae.httpSend(configDocUrl, {
        ...
        error: function (code, error, reason) {
          $.showMessage({
            title: "error",
            content: "Impossible de charger config document : " + configDocUrl,
          });
        },
      });
    },
    loadConfigJson: function (configDocUrl) {
      $.ajax({
        url: configDocUrl,
        dataType: "json",
        async: false,
    ...
    getItem: function (itemUrl, callback, success, error) {
      if (!error)
        error = function (code, error, reason) {
          $.showMessage({
            title: "error",
            content: "Impossible de charger d'item : " + itemUrl,
          });
        };
      if (!success)
        success = function (doc) {
    ...
    getViewpoint: function (viewpointUrl, callback) {
      $.agorae.httpSend(viewpointUrl, {
        ...
        error: function (code, error, reason) {
          $.showMessage({
            title: "error",
            content: "Cannot load viewpoint:" + viewpointUrl,
          });
        },
      });
    },
    ...
    getUser: function (serverUrl, callback) {
      this.httpSend(serverUrl + "user/" + $.agorae.session.username, {
        ...
        success: function (doc) {
          ...
          if (user.corpus) $.agorae.config.corpus = user.corpus;
          callback(user);
        },
        error: function (code, error, reason) {
          $.showMessage({
    ...
    createViewpoint: function (viewpointName, callback) {
      ...
      $.agorae.httpSend($.agorae.config.servers[0], {
        ...
        error: function (code, error, reason) {
          $.showMessage({
            title: "Erreur",
            content: "Impossible de cr¨¦er un point de vue : " + reason,
          });
        },
      });
    },
      ...
      $.agorae.httpSend(viewpointUrl, {
        ...
        success: function (viewpoint) {
          ...
          success({
            ...
            name: topic.name,
            narrower: narrower,
            viewpoint_name: viewpoint.name[0],
      ...
      $.agorae.httpSend(url, {
        ...
        success: function (viewpoint) {
          ...
          for (var i = 0, topic; (topic = topics[i]); i++) {
            ...
            if (!viewpoint.topics[topic.id].broader)
              viewpoint.topics[topic.id].broader = [];
            if (viewpoint.topics[topic.id].broader.indexOf(topicID) >= 0)
              continue;
    ...
    moveTopicIn2: function (url, topicID, parentTopicID) {
      ...
      $.agorae.httpSend(documentUri, {
        ...
        success: function (viewpoint) {
          if (parentTopicID) {
            if (!viewpoint.topics[topicID].broader)
    ...
    getUserCorpus: function (url, success) {
      ...
      $.agorae.httpSend(userUrl, {
        ...
        error: function (code, error, reason) {
          $.showMessage({
            title: "error",
            content: "Cannot load user:" + username + " from server:" + url,
          });
        },
      });
    },
    createCorpus: function (url, corpusName, success) {
      ...
      $.agorae.httpSend(url, {
        ...
        error: function (code, error, reason) {
          $.showMessage({
            title: "Erreur",
            content: "Impossible de créer corpus : " + reason,
          });
        },
      });
    },
    ...
    createItemWithTopicAndCorpus: function (
      topicUrl,
      corpusUrl,
      name,
      success
    ...
    deleteItemAttachment: function (itemUrl, attachment_name, callback) {
      $.agorae.httpSend(itemUrl, {
        ...
        success: function (doc) {
          $.agorae.httpSend(
            itemUrl + "/" + attachment_name + "?rev=" + doc._rev,
            {
              type: "DELETE",
              success: function (doc) {
                callback(doc);
    ...
    deleteItemAttribute: function (
      ...
      $.agorae.httpSend(itemUrl, {
        ...
        success: function (doc) {
          if (
            !doc[attributename] ||
            doc[attributename].indexOf(attributevalue) < 0
          ) {
            success();
            return false;
          }
      ...
      $.agorae.httpSend(itemUrl, {
        ...
        success: function (item) {
          if (!item[name]) {
            ...
            if (typeof item[name] == "string") item[name] = [item[name], value];
            else item[name].push(value);
          }
          $.agorae.httpSend(itemUrl + "?rev=" + item._rev, {
            type: "PUT",
            data: item,
    ...
    undescribeItem: function (itemUrl, name, value, success) {
      ...
      $.agorae.httpSend(itemUrl, {
        ...
        success: function (item) {
          ...
          if (typeof item[name] == "string") delete item[name];
          else {
            ...
            if (item[name].length == 0) delete item[name];
            else if (item[name].length == 1) item[name] = item[name][0];
          }
          $.agorae.httpSend(itemUrl + "?rev=" + item._rev, {
            type: "PUT",
            data: item,
    ...
    getBroader: function (viewpoint, topicID) {
      var result = [];
      if (
        topicID in viewpoint.topics &&
        typeof viewpoint.topics[topicID].broader != "undefined"
      ) {
        $.merge(result, viewpoint.topics[topicID].broader);
        for (
          var i = 0, parentID;
          (parentID = viewpoint.topics[topicID].broader[i]);
          i++
        ) {
    ...
    getTopicTree: function (viewpointUrl) {
      ...
      if (!viewpointUrl) {
        if ($.agorae.session.username)
          $.agorae.httpSend(
            $.agorae.config.servers[0] + "user/" + $.agorae.session.username,
            {
              type: "GET",
              async: false,
              success: function (doc) {
                ...
                if (user.viewpoint)
                  for (
                    var i = 0, viewpoint;
                    (viewpoint = user.viewpoint[i]);
                    i++
                  )
                    viewpoints.push(
                      $.agorae.config.servers[0] + "viewpoint/" + viewpoint.id
                    );
              },
            }
          );
        if ($.agorae.config.viewpoints)
          for (var i = 0, url; (url = $.agorae.config.viewpoints[i]); i++)
            if (viewpoints.indexOf("http") == 0) viewpoints.push(url);
            else
              viewpoints.push($.agorae.config.servers[0] + "viewpoint/" + url);
      } else viewpoints.push(viewpointUrl);
      ...
      $.each($.agorae.config.servers, function (idx, server) {
        ...
        $.agorae.httpSend(url, {
          ...
          error: function (code, error, reason) {
            $.showMessage({
              title: "error",
              content: "Cannot load corpus:" + url,
            });
          },
        });
      });
    ...
    loadDisqus: function () {
      (function () {
        ...
        s.async = true;
        s.id = "dsq_js";
        s.src = $.agorae.config.disqus;
        s.setAttribute("data-timestamp", +new Date());
        (d.head || d.body).appendChild(s);
      })();
    },
    insertDisqus: function () {
      $("<div id='disqus_thread'></div>").insertAfter("#bDis");
    },
    resetDisqus: function (newIdentifier, newUrl, name) {
      DISQUS.reset({
        reload: true,
        config: function () {
          this.page.identifier = newIdentifier;
          this.page.url = newUrl + "#!newthread";
          this.page.title = name;
          this.language = $("#languages").val();
        },
      });
    },
    ...
    searchItem: function (uris) {
      ...
      $.each(uris, function (idx, uri) {
        $.agorae.httpSend(uri, {
          ...
          success: function (doc) {
            ...
            for (corpus in doc) break;
            if (!corpus) return;
            doc = doc[corpus];

            for (attributename in doc) break;
            if (!attributename) return;
            doc = doc[attributename];

            for (attributevalue in doc) break;
            ...
            for (var i = 0; (item = doc.item[i]); i++) {
              ...
              if (item.id in _items) _items[item.id]++;
              else _items[item.id] = 1;
            }
          },
        });
      });
      ...
      for (var i = 0, item; (item = items[i]); i++)
        if (item.item in _items && _items[item.item] == uris.length) {
          if (item.item in unique) continue;
          else {
            unique[item.item] = {};
            result.push(item);
          }
