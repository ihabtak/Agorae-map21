(function ($) {
  ...
  function PageHelper() {
    this.init = function (config) {
      ...
      //init event;
      $("a#item-search").click(function () {
        $.agorae.itemdialog.open(shortcutToItem);
        return false;
      });
      $("a#sign-in").click($.agorae.session.login);
      $("a#sign-out").click($.agorae.session.logout);
      $("a#shortcut").click(function () {
        $.agorae.topictree.openDialog(shortcutToTopic);
        return false;
      });
    ...
    this.navigatorBar = function (obj) {
      if (typeof obj != "string") {
        ...
        for (var i = 0, bar; (bar = obj[i]); i++) {
          if (bar.uri) str += '<a href="' + bar.uri + '">' + bar.name + "</a>";
          else str += "<b>" + bar.name + "</b>";
    ...
    function resizeSidebar() {
      ...
      viewportHeight =
        viewportHeight < $("div#main").outerHeight()
          ? $("div#main").outerHeight()
          : viewportHeight;
    ...
    function shortcutToTopic() {
      ...
      if ($("#tt-tree").prop("checked"))
        ...
        for (var j = 0, el; (el = $("#tags a.tag-clicked")[j]); j++) {
          ...
          for (var k = 0, t; (t = tag_topics[k]); k++) uris.push(t.uri);
        }
      $.agorae.topictree.closeDialog();
      if (uris.length == 0) return;
      if (uris.length == 1) $.agorae.pagehelper.rewrite(uris[0]);
      else
        for (var i = 0, uri; (uri = uris[i]); i++)
          if (i == 0) $.agorae.pagehelper.rewrite(uris[i]);
          else window.open(uris[i]);
    }
    ...
    this.login = function () {
      $.showDialog("dialog/_login.html", {
        ...
        load: function (dialog) {
          ...
          $(dialog)
            .find('input[name="cookie"]')
            .prop("checked", cookie == "1");
          $(dialog).i18n();
        },
  ...
  function IndexPage() {
    this.init = function () {
      if (!$.agorae.config.servers) {
        $.showMessage({
          title: $.i18n("Erreur"),
          content: $.i18n(
            "Agorae est mal configuré : le serveur Hypertopic n'est pas indiqué."
          ),
        });
        return false;
      }
      if ($.agorae.config.corpora) $.each($.agorae.config.corpora, getCorpus);
      ...
      $.agorae.pagehelper.navigatorBar("<b>" + $.i18n("Tables") + "</b>");
      ...
      $("div.index ul#viewpoint li[uri] span")
        .die()
        .live("click", onViewpointClick);

      // I18n update
      // if ($.i18n("agorae-introduction"))
      //   $("#introduction").html($.i18n("agorae-introduction"));
      $("body").i18n();
    };
    function getCorpus(idx, corpus) {
      if (corpus.indexOf("http") != 0)
        corpusUrl = $.agorae.config.servers[0] + "corpus/" + corpus;
      else corpusUrl = corpus;
      $.agorae.getCorpus(corpusUrl, function (corpus) {
        var el = $("<li><span>" + corpus.name + "</span></li>")
          .attr("id", corpus.id)
          .attr("uri", corpusUrl);
        $("ul#corpus").append(el);
      });
    ...
    function appendViewpoint(idx, viewpointUrl, viewpoint, editable) {
      if (!viewpoint) {
        ...
        if (viewpointUrl.indexOf("http") != 0)
          viewpointUrl =
            $.agorae.config.servers[0] + "viewpoint/" + viewpointUrl;
        $.agorae.getViewpoint(viewpointUrl, showViewpoint);
      } else {
        $("li#" + viewpoint.id, "ul#viewpoint")
          .hide()
          .remove();
        var el = $(
          '<li><img class="del ctl hide" src="css/blitzer/images/delete.png">' +
            "<span>" +
            viewpoint.name +
            "</span></li>"
        )
          ...
          .data("viewpoint", viewpoint)
          .attr("uri", viewpointUrl);
        if (editable) el.find("span").addClass("editable");
        $("ul#viewpoint").append(el);
      }
    }
      ...
      $.agorae.getUser(server, function (user) {
        ...
        if (user.corpus)
          for (var i = 0, corpus; (corpus = user.corpus[i]); i++) {
            ...
            var el = $(
              '<li><img class="del ctl hide" src="css/blitzer/images/delete.png"><span class="editable">' +
                corpus.name +
                "</span></li>"
            )
              .attr("id", corpus.id)
    ...
    function onCorpusClick() {
      ...
      el.keyup(function (event) {
        if (event.keyCode == 27 || event.keyCode == 13) $(this).blur();
      });
      return false;
    }
    ...
    function createViewpoint() {
      $.agorae.createViewpoint($.i18n("Sans nom"), function (doc) {
        appendViewpoint(
          0,
          $.agorae.config.servers[0] + "viewpoint/" + doc.id,
          doc,
          true
        );
        $.agorae.pagehelper.checkController();
  ...
  function CorpusPage() {
    this.init = function () {
      ...
      if (
        typeof $.agorae.config.servers[0] == "string" &&
        uri.indexOf($.agorae.config.servers[0]) == 0
      )
        $.agorae.pagehelper.toggle(true);
      else $.agorae.pagehelper.toggle(false);
        ...
        $.agorae.getItem(
          ...
          function () {
            if ($.agorae.config.servers)
              for (
                var i = 0, server;
                (server = $.agorae.config.servers[i]);
                i++
              ) {
  ...
  function ViewpointPage() {
    this.init = function () {
      ...
      $.agorae.getViewpoint(uri, function (viewpoint) {
        ...
        $.agorae.pagehelper.navigatorBar(bars);
        if (viewpoint.upper) $.each($.sortByName(viewpoint.upper), appendTopic);
      });
      ...
      $("div.viewpoint div.topic-list img.del")
        .die()
        .live("click", $.agorae.viewpointpage.deleteTopic);
      $("div.viewpoint ul#topic li[uri] span")
        .die()
        .live("click", $.agorae.viewpointpage.onTopicClick);
  ...
  function getTopicPath(uri, callback) {
    $.agorae.getTopic(uri, function (topic) {
      ...
      path.unshift({
        uri: "#" + topic.viewpointUrl,
        name: topic.viewpoint_name,
      });
      callback(path, topic);
    });
  }
  function TopicPage() {
    this.init = function () {
      ...
      getTopicPath(uri, function (path, topic) {
        path.unshift({ uri: "#", name: $.i18n("Tables") });
        $.agorae.pagehelper.navigatorBar(path);
        if (topic.narrower) $.each($.sortByName(topic.narrower), appendTopic);
        if (topic.item) $.each($.sortByName(topic.item), appendItem);
      });
      ...
      $("div.topic div.topic-list img.del")
        .die()
        .live("click", $.agorae.viewpointpage.deleteTopic);
      $("div.topic div.topic-list img.unlink").die().live("click", unlinkTopic);
      $("div.topic ul#topic li[uri] span")
        .die()
        .live("click", $.agorae.viewpointpage.onTopicClick);
      ...
      $("div.topic div.item-list img.attach")
        .die()
        .live("click", function () {
          $.agorae.itemdialog.open(linkItem);
          return false;
        });
    ...
    function attachTopic() {
      $.agorae.topictree.openDialog(function () {
        var topics = [];
        if ($("#tt-tree").prop("checked"))
          for (
            var i = 0, t;
            (t = $.jstree._focused().data.ui.selected[i]);
            i++
          ) {
            if ($(t).attr("rel") == "viewpoint") continue;
            topics.push({
              id: $(t).attr("topicID"),
              viewpoint: $(t).attr("viewpointID"),
              name: $(t).attr("name"),
              uri: $(t).attr("uri"),
            });
          }
        ...
        if (topics.length == 0) {
          $.showMessage({
            title: "Attention",
            content: "Vous devez d'abord sélectionner un thème.",
          });
          return false;
        }
        var uri = $.getUri();
      ...
      $.showDialog("dialog/_item.html", {
        ...
        load: function (dialog) {
          ...
          $.agorae.getCorpora(function (corpus) {
            if (
              corpusSelect.find('option[uri="' + corpus.uri + '"]').length > 0
            )
              return;
        ...
        submit: function (data, callback) {
          ...
          var corpusOption = $(
            'select[name="corpus"] option[value="' + corpus + '"]'
          );
          var corpusName = corpusOption.text();
          var corpusUrl = corpusOption.attr("uri");

          $.agorae.createItemWithTopicAndCorpus(
            uri,
            corpusUrl,
            data.itemName,
            function (item) {
  ...
  function ItemPage() {
    this.init = function () {
      ...
      $.agorae.getItem(uri, function (item) {
        ...
        item_name = item.name;
        if (item.resource) appendRemoteResource(item.resource);
        if (item._attachments) appendAttachment(item._attachments[0]);
        if (item.topic) $.each(item.topic, appendTopic);
        if (item.corpus) {
      ...
      else $.agorae.pagehelper.toggle(false, onEditOn, onEditOff);

      $("div.item div.remote-resource-list img.add").bind(
        "click",
        attachRemoteResource
      );
      $("div.item div.remote-resource-list img.unlink")
        .die()
        .live("click", unlinkRemoteResource);
      $("div.item div.remote-resource-list span.resource")
        .die()
        .live("click", clickRemoteResource);

      $("div.item div.local-resource-list li.attribute img.del")
        .die()
        .live("click", deleteAttribute);
      $("div.item div.local-resource-list li.attachment img.del")
        .die()
        .live("click", deleteAttachment);
      $("div.item div.local-resource-list img.add").bind(
        "click",
        addLocalResource
      );
      $("div.item div.local-resource-list img.upload").bind(
        "click",
        uploadAttachment
      );
      ...
      $("div.item div.attribute-list img.del")
        .die()
        .live("click", deleteAttribute);
      $("div.item div.attribute-list span.editable")
        .die()
        .live("click", updateAttributeValue);

      $("div.item ul#topic li[uri] span")
        .die()
        .live("click", $.agorae.viewpointpage.onTopicClick);
    ...
    function onEditOn() {
      ...
      $("div.attribute").each(function () {
        ...
        attributes[name] = [];
        $(this)
          .find(".attributevalue span")
          .each(function () {
            attributes[name].push($(this).attr("attributevalue"));
          });
      });
      $("ul#attribute").html("");
      for (var name in attributes) appendAttribute(name, attributes[name]);
      $("div.attributes").hide();
      $("ul#attribute").show();
      ...
      else $("div.remote-resource-list img.add").addClass("ctl").show();
    }
    function onEditOff() {
      var attributes = {};
      ...
      for (var name in attributes) {
        var nameW = name.substring(4);
        var el_attribute = $('<div class="attribute">');
        var el_name = $(
          '<div style="display: inline;" class="attributename"></div>'
        )
          .text(nameW)
          .attr("attributename", name);
        var el_values = $(
          '<div style="display: inline;" class="attributevalue"></div>'
        );
        for (var i = 0, v; (v = attributes[name][i]); i++) {
          var comma = i < attributes[name].length - 1 ? "," : "";
          var span_value = $("<span></span>")
      ...
      $("div.attribute")
        .filter(function () {
          return (
            $(this).find("span[attributevalue]").text() === "corpusAttribute"
          );
        })
        .hide();
      $("ul#local-resource li").length > 0
        ? $("div.local-resource-list").show()
        : $("div.local-resource-list").hide();
      $("ul#remote-resource li").length > 0
        ? $("div.remote-resource-list").show()
        : $("div.remote-resource-list").hide();
    }
    function parseUri() {
      var uri = $.getUri();
    ...
    function updateAttributeValue() {
      ...
      var uri = $.getUri();
      var uris = parseUri();
      var corpusAttributeValues = Object.keys(
        $.agorae.getAttributeValue(
          [uris.prefixUrl + "attribute/" + uris.corpus],
          attributename
        )
      );
      var el = $('<input type="textbox">')
        .val(attributevalue)
        .autocomplete({ source: corpusAttributeValues });
    ...
    function appendAttribute(name, value) {
      ...
      for (var i = 0, v; (v = value[i]); i++) {
        var el = $(
          '<li><img class="del ctl hide" src="css/blitzer/images/delete.png"></li>'
        )
          .attr("attributename", name)
          .attr("attributevalue", v);
        ...
        var protocol = "";
        try {
          protocol = $.url.setUrl(v).attr("protocol");
        } catch (e) {}
        if (
          protocol == "http" ||
          protocol == "https" ||
          ...
          if(name=="image/video"){
            var img_vidItem ;
            var sVid = $("<div class='plyr__video-embed' id='player'></div>");
            if(checkURL(v)){
              img_vidItem = $("<img></img>");
              img_vidItem.attr("src", v)
              $("#imageVideoItem").append(img_vidItem);
            }else{
              if(v.includes("youtu") || v.includes("vimeo") ){
                var ifr = $("<iframe allowfullscreen allowtransparency allow='autoplay'></iframe>");
                ifr.attr("src", v)
                sVid.append(ifr);
                $("#imageVideoItem").append(sVid);
              }else{
                img_vidItem = $("<video id='player' playsinline controls></video>");
                img_vidItem.attr("src", v)
                $("#imageVideoItem").append(img_vidItem);
              }
            }
          ...
          if( name=="icône" || name=="icone"  ){
            var iconItem ;
              iconItem = $("<img width='300' height='300' ></img>")
            iconItem.attr("src", v)
           $("#iconItem").append(iconItem);
          }
          el.append(el_href);
          ...
          //Is noraml attribute
          var span_name = $(
            '<span class="editable attributename"></span>'
          ).text(name);
          var span_value = $(
            '<span class="editable attributevalue"></span>'
          ).text(v);
          if (v === "corpusAttribute") {
            span_value = $(
              '<span class="editable attributevalue"></span>'
    ...
    function appendAttachment(attachments) {
      ...
      for (var file in attachments) {
        ...
        var contentType =
          typeof attachments[file].content_type == "string"
            ? attachments[file].content_type
            : "";
        var fancybox_group =
          contentType.indexOf("image/") == 0 ? "fancybox_group" : null;
        ...
        var el = $(
          ...
          .addClass("attachment")
          .attr("file", file);
        var el_href = $('<a class="editable attachment"></a>')
          .attr("href", uri)
          .html(file);
        if (fancybox_group) el_href.attr("rel", fancybox_group);
        el.append(el_href);
        $("ul#local-resource").append(el);
      }
      fbox();
      ...
      $.showDialog("dialog/_resource.html", {
        ...
        submit: function (data, callback) {
          if (!data.resource || data.resource.length == 0) {
            callback({
              resource: "L'aderesse de la ressource ne doit pas être vide!",
            });
            return;
          }
    ...
    function appendRemoteResource(resource) {
      ...
      var el_container = $("<li></li>").append(
        $('<img class="unlink ctl hide" src="css/blitzer/images/unlink.png">')
      );
      var span_resource = $('<span class="editable resource"></span>')
        .html(resource)
        .attr("rel", resource);
    ...
    function addLocalResource() {
      $.showDialog("dialog/_localresource.html", {
        ...
        submit: function (data, callback) {
          if (!data.attributename || data.attributename.length == 0) {
            callback({
              attributename: "Veuillez saisir le nom de la ressource",
            });
            return;
          }
          if (!data.attributevalue || data.attributevalue.length == 0) {
            callback({
              attributename: "Veuillez saisir l'aderesse de la ressource",
            });
            return;
          }
          var protocol;
          try {
            protocol = $.url.setUrl(data.attributevalue).attr("protocol");
          } catch (e) {}
          if (
            protocol != "http" &&
            protocol != "https" &&
            ...
            callback({
              attributevalue: "L'aderesse de la ressource n'est pas correct!",
            });
            return;
          }
          uri = $.getUri();
    ...
    function detachTopic() {
      ...
      $.agorae.untagItem(itemUrl, id, function () {
        $('div.item div.topic-list ul#topic li[id="' + id + '"]')
          .hide()
          .remove();
      });
    }
    function appendTopic(idx, topic) {
      ...
      if (topic.uri) {
        getTopicPath(topic.uri, function (path) {
          var html =
            '<li><img class="unlink ctl hide" src="css/blitzer/images/unlink.png">';
          for (var i = 0; i < path.length - 1; i++)
            html += path[i].name + " &gt; ";
          html += '<span class="editable">' + topic.name + "</span></li>";
          var el = $(html)
            .attr("id", topic.id)
            .attr("viewpoint", topic.viewpoint)
            .attr("uri", topic.uri);
          $("ul#topic").append(el);
        });
      }
  ...
  function TopicTree() {
    ...
    function showTagCloud(tags) {
      ...
      for (var name in tags) {
        ...
        var content = $("<li class='tag" + size + "'><a>" + name + "</a></li>")
          .attr("topics", JSON.stringify(tags[name].topics))
          .attr("name", name);
        $("#tags ul").append(content);
      }
      $("#tags ul li").tsort();
      $("#tags ul li a").click(function () {
        if ($.agorae.session.ctrlPressed) $(this).toggleClass("tag-clicked");
        else {
    ...
    this.openDialog = function (callback) {
      ...
      $("#topic-tree-dialog")
        .dialog("option", "buttons", labelAction)
        .dialog("open");
    };
      ...
      if (availableLanguages.length > 1) {
        ...
        for (idx in availableLanguages) {
          if (availableLanguages.hasOwnProperty(idx)) {
            ...
            var opt = $("<option>" + lang.toUpperCase() + "</option>").val(
              lang
            );
            if ($.i18n().locale === lang) {
  ...
  function ItemDialog() {
    this.init = function () {
      ...
      $("ul.search-condition select.attributename")
        .die()
        .live("change", onNameChange);
      $("ul.search-condition button.plus")
        .die()
        .live("click", showSearchCondition);
      $("ul.search-condition button.minus")
        .die()
        .live("click", removeSearchCondition);
    };
    ...
    function showSearchCondition() {
      ...
      var el = $(
        '<li style="display: none">' +
          $.i18n("Nom :") +
          ' <select class="attributename"><option value=""></option></select>' +
          " " +
          $.i18n("Valeur :") +
          '<select class="attributevalue"><option value=""></option></select>' +
          '<button class="plus"></button><button class="minus"></button></li>'
      ).attr("id", uuid);
    ...
    function onNameChange() {
      ...
      if (name == "") return;
      var values = $.agorae.getAttributeValue(
        $.agorae.itemdialog.names[name],
        name
      );
      for (var v in values) {
    ...
    function doSearch() {
      if ($("#item-search-condition").css("display") != "none") {
        ...
        $("select.attributevalue", "ul.search-condition").each(function () {
          ...
          for (var i = 0, v; (v = values[i]); i++) {
            var uri =
              v.uri +
              "/" +
              encodeURIComponent(v.attributename) +
              "/" +
              encodeURIComponent(v.attributevalue);
        ...
        $(this)
          ...
          .html($.i18n("Étape précédente"));
        $("#item-search-result")
          .show()
          .html(
            '<img src="css/blitzer/images/loading.gif" style="padding-left: 1em;">'
          );
